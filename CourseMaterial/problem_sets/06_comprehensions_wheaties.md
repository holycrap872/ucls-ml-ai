...
0. Create a function that takes a list of tuples (name, age) as an input and
   returns a dictionary where the keys are the names and the values are the ages.
    - For example: `[("Alice", 25), ("Bob", 30)] -> {"Alice": 25, "Bob": 30}`
0. Create a function that takes a dictionary and returns a new dictionary where
...
0. Create a function that takes two lists of equal size and returns a dictionary
